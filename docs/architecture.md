...
export class Conversation {
  ...
  @Prop({ type: Object })
  lastMessage?: {
    // денормализация для списка чатов (без доп. запроса)
...
async function storePrivateKey(privateKey: Uint8Array, pin: string) {
  ...
  const encrypted = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, pinKey, privateKey);
...
_Документ подготовлен как стартовая точка. Все технические решения покрывают MVP и заложены с расчётом на рост до ~10K DAU без переписывания архитектуры._
